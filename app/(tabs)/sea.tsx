...
const SEA_CATEGORIES = ['Ferry', 'FastCraft', 'Bangka', 'Cruise'] as const;
...
const SEA_TRANSPORTS = [
  ...
  },
  {
    ...
    color: '#06B6D4',
    description: 'Small outrigger boats for island-hopping, snorkeling tours, and short coastal crossings.',
  ...
  },
  {
    ...
    color: '#7C3AED',
    description: 'Luxury cruise ships for leisure travel. Growing in popularity across the Philippine archipelago.',
...
function createSeedItinerary(trip: Trip): ItineraryItem[] {
  return [
    { id: `${trip.id}-i1`, dayIndex: 0, time: '07:00', category: 'Transport', title: `${trip.transportType} to ${trip.destination}`, location: `From ${trip.origin}`, cost: Math.round(trip.totalCost * 0.4) },
    { id: `${trip.id}-i2`, dayIndex: 0, time: '13:00', category: 'Food', title: `Lunch in ${trip.destination}`, location: trip.destination, cost: Math.round(trip.totalCost * 0.1) },
  ];
}
...
export default function SeaTransportScreen() {
  ...
  const seaTrips = useMemo(
    () => [...trips]
      .filter((t) => (SEA_CATEGORIES as readonly string[]).includes(t.transportType))
  ...
  const totalKm = seaTrips.reduce((s, t) => s + t.distanceKm, 0);
  const totalSpent = seaTrips.reduce((s, t) => s + t.totalCost, 0);
  const avgCost = seaTrips.length > 0 ? Math.round(totalSpent / seaTrips.length) : 0;
  const totalTravelers = seaTrips.reduce((s, t) => s + t.passengers, 0);
  const completedCount = seaTrips.filter(t => t.status === 'completed').length;
  const confirmedCount = seaTrips.filter(t => t.status === 'confirmed').length;
  const plannedCount = seaTrips.filter(t => t.status !== 'completed' && t.status !== 'confirmed' && t.status !== 'cancelled').length;
  const cancelledCount = seaTrips.filter(t => t.status === 'cancelled').length;
  const typeBreakdown = (SEA_CATEGORIES as readonly string[])
    .map(cat => ({ type: cat, count: seaTrips.filter(t => t.transportType === cat).length }))
    .filter(x => x.count > 0)
  ...
  return (
    <LinearGradient colors={[ranaColors.backgroundTop, ranaColors.backgroundBottom]} style={styles.container}>
      <TouchableOpacity
        ...
        onPress={() => router.push('/new-trip-sea')}
        activeOpacity={0.85}
      >
        <Ionicons name="add" size={16} color="#fff" />
        <Text style={styles.newTripBtnText}>New Sea Trip</Text>
      </TouchableOpacity>
          ...
          <View>
            ...
            <View style={styles.dashCard}>
              ...
              <View style={styles.metricRow}>
                <View style={styles.metricItem}>
                  <View style={[styles.metricIcon, { backgroundColor: '#E0F2FE' }]}>
                    <Ionicons name="speedometer-outline" size={13} color={SEA_ACCENT} />
                  </View>
                ...
                <View style={styles.metricItem}>
                  <View style={[styles.metricIcon, { backgroundColor: '#CFFAFE' }]}>
                    <Ionicons name="boat-outline" size={13} color="#06B6D4" />
                  </View>
                  <Text style={styles.metricVal}>{seaTrips.length}</Text>
                  ...
                  {[
                    ...
                    <View key={i} style={styles.barRow}>
                      ...
                      <View style={styles.barTrack}>
                        <View style={{ flex: s.count / seaTrips.length, backgroundColor: s.color, height: 6, borderRadius: 3 }} />
                        <View style={{ flex: Math.max(0, 1 - s.count / seaTrips.length) }} />
                      </View>
              ...
              {typeBreakdown.length > 0 && (
                <>
                  <View style={styles.dashDiv} />
                  <Text style={styles.dashSubhead}>By Vessel Type</Text>
                  {typeBreakdown.map((item, i) => (
                    <View key={i} style={styles.barRow}>
                      <Text style={styles.barLbl}>{item.type}</Text>
                      <View style={styles.barTrack}>
                        <View style={{ flex: item.count / maxTypeCount, backgroundColor: SEA_ACCENT, height: 6, borderRadius: 3 }} />
            ...
            <Text style={styles.sectionTitle}>Vessel Types</Text>
            ...
            {POPULAR_ROUTES.map((route, i) => (
              <View key={i} style={styles.routeCard}>
                <View style={styles.routeLeft}>
                  ...
                  <Ionicons name="arrow-forward" size={14} color={SEA_ACCENT} />
            ...
            {seaTrips.length === 0 && (
              <View style={styles.emptyState}>
                <View style={styles.emptyIconWrap}>
                  <Ionicons name="boat-outline" size={28} color={ranaColors.muted} />
                </View>
                <Text style={styles.emptyTitle}>No sea trips yet</Text>
                <Text style={styles.emptyText}>Tap "New Sea Trip" above to log your first voyage.</Text>
              </View>
            )}
          </View>
        }
              ...
              <View style={styles.cardInner}>
                <View style={styles.tripHeader}>
                  <View style={styles.transportWrap}>
                    <View style={[styles.transportIconCircle, { backgroundColor: SEA_ACCENT }]}>
                      <Ionicons name="boat" size={11} color="#fff" />
                    </View>
          ...
          {activeTrip && (
            <>
              <View style={styles.detailHeaderRow}>
                ...
                <Text style={styles.detailHeaderTitle}>Sea Trip</Text>
                <View style={{ width: 36 }} />
              </View>
                  ...
                  {tripDays[activeDayIndex] && (
                    <View style={styles.dayHeaderBlock}>
                      <View style={styles.dayHeaderTitleRow}>
                        <View style={{ flex: 1 }}>
                          <Text style={styles.dayHeaderTitle}>{`Day ${activeDayIndex + 1} - ${activeDayIndex === 0 ? 'Departure' : 'Voyage'}`}</Text>
...
const styles = StyleSheet.create({
  ...
  newTripBtn: {
    ...
    backgroundColor: SEA_ACCENT, ...ranaShadow.card,
  },
  ...
  spendBand: {
    ...
    backgroundColor: '#F0F9FF', borderRadius: 12, padding: 12, marginBottom: 2,
  },
  ...
  spendAvgVal: { fontSize: 13, fontWeight: '700', color: SEA_ACCENT },
  ...
  barTrack: { flex: 1, height: 6, backgroundColor: '#F0F9FF', borderRadius: 3, flexDirection: 'row', overflow: 'hidden' },
  barCnt: { fontSize: 11, fontWeight: '700', color: ranaColors.textSecondary, width: 16, textAlign: 'right' },
  emptyIconWrap: { width: 52, height: 52, borderRadius: 14, backgroundColor: '#F0F9FF', alignItems: 'center', justifyContent: 'center' },
  statsRow: { flexDirection: 'row', gap: 8, marginBottom: 16 },
  contextCard: {
    flexDirection: 'row', alignItems: 'flex-start', gap: 10,
    backgroundColor: '#F0F9FF', padding: 14, borderRadius: 16, marginBottom: 20,
  ...
  exampleChip: { backgroundColor: '#F0F9FF', paddingHorizontal: 8, paddingVertical: 2, borderRadius: 8 },
  exampleText: { fontSize: 10, fontWeight: '600', color: SEA_ACCENT },
  sectionTitle: { fontSize: 18, fontWeight: '800', color: ranaColors.textPrimary, marginTop: 8, marginBottom: 12 },
  routeCard: {
    flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center',
    backgroundColor: '#FFFFFF', borderRadius: 16, padding: 14, marginBottom: 8,
    borderWidth: 1, borderColor: '#E8EEF9',
  },
  routeLeft: { flexDirection: 'row', alignItems: 'center', gap: 6, flex: 1 },
  routeFrom: { fontSize: 13, fontWeight: '700', color: ranaColors.textPrimary },
  routeTo: { fontSize: 13, fontWeight: '600', color: ranaColors.textSecondary },
  routeRight: { alignItems: 'flex-end', gap: 2 },
  routeTypeChip: { backgroundColor: '#F0F9FF', paddingHorizontal: 8, paddingVertical: 2, borderRadius: 8 },
  routeTypeText: { fontSize: 10, fontWeight: '700', color: SEA_ACCENT },
  routeDuration: { fontSize: 11, color: ranaColors.textSecondary, fontWeight: '600' },
  ...
  trackBtnText: { fontSize: 11, fontWeight: '700', color: '#0284C7' },
  ...
  dayChipActive: { backgroundColor: SEA_ACCENT },
  ...
  addRowBtn: {
    flexDirection: 'row', alignItems: 'center', gap: 6, backgroundColor: SEA_ACCENT,
  ...
  timelineTime: { fontSize: 12, fontWeight: '700', color: SEA_ACCENT },
  removeBtn: { padding: 2 },
  ...
  formCategoryChipActive: { backgroundColor: '#E0F2FE', borderColor: SEA_ACCENT },
  formCategoryText: { fontSize: 12, fontWeight: '600', color: ranaColors.textSecondary },
  formCategoryTextActive: { color: SEA_ACCENT },
  formSaveBtn: { backgroundColor: SEA_ACCENT, borderRadius: 14, padding: 14, alignItems: 'center', marginTop: 16 },
